...
const Modal = ({ activeId, setShowModal }) => {
  ...
  return (
    <div className="w-full h-full fixed top-0 left-0 z-10 bg-headingColor bg-opacity-40">
      <div className="w-11/12 md:max-w-md sm:max-w-xs   absolute top-1/2 md:top-[55%] sm:top-[75%] left-1/2 z-20 bg-white dark:bg-gray-800 rounded-[8px] transform -translate-x-1/2 -translate-y-1/2 p-5">
        <div>
          <figure>
            <img className="rounded-[8px]" src={portfolio.imgUrl} alt="" />
          </figure>
        </div>
        <h2 className="text-2xl md:text-xl text-headingColor dark:text-primaryColor font-[700] my-5 md:my-1">
          {portfolio.title}
        </h2>
        ...
        <div className="mt-5 md:mt-0 flex items-center gap-3 flex-wrap">
          ...
          {portfolio.technologies.map((item, index) => (
            <span
              className="bg-gray-200 dark:bg-gray-600 py-1  md:py-0 px-2 md:px-0 rounded-[5px] text-[14px] leading-0"
              key={index}
            >
              {item}
            </span>
